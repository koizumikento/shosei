...
現在のリポジトリは spec-first で進んでおり、公開 docs、Rust 実装、CI smoke が同じ command surface を説明・証明する形で進めています。
...
`v0.2` は、主要 CLI workflow を継続検証しながら使える段階です。外部 validator は tool の入手性に応じて段階的に扱い、portable な CI smoke と opt-in の実環境検査を分けています。

現在の readiness:

| Area | Status | Evidence |
|---|---|---|
| CLI workflow | supported | `init`, `explain`, `build`, `validate`, `preview`, `doctor`, `handoff`, `series sync`, `page check`, `reference`, `story` を実装し、CLI smoke で継続確認 |
| Repository model | supported | `single-book` と `series`、`book.yml` / `series.yml` の探索と解決を実装 |
| Delivery validation | supported with explicit evidence | local lint、`epubcheck`、`qpdf`、opt-in Kindle Previewer conversion check を `validators[]` と `delivery_evidence` に記録 |
| Handoff package | supported | artifact、`reports/validate.json`、`delivery_evidence` 付き `manifest.json`、proof review packet、editorial sidecar を package 化 |
| VS Code adapter | supported thin adapter | CLI に実処理を委譲し、adapter check / unit / host smoke / packaged VSIX smoke を CI で継続確認 |
| Store/device-specific validation beyond Kindle Previewer | future work | Kindle Previewer 以外の深い device / store-specific validator は未実装 |
...
- opt-in Kindle validate での Kindle Previewer conversion check 記録
...
実物 Kindle Previewer が入っている release host では、必須 CI とは別に次の opt-in hook で real-tool conversion evidence を残せる。
