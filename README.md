...
- target/profile ごとの command-level smoke 拡張の残り
...
GitHub Actions では CLI smoke を `ubuntu-latest`, `macos-latest`, `windows-latest` の 3 OS matrix で回し、VS Code adapter は `npm ci` の後に `npm run check`, `npm test`, `npm run test:host` を 3 OS で、`npm run test:package-smoke` を Ubuntu で継続確認する。
