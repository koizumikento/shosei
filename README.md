...
`init` は現在、短い対話式です。既定値で一気に scaffold を作る場合は `--non-interactive --config-template <template>` を使えます。利用できるテンプレートは `business`, `paper`, `novel`, `light-novel`, `manga` です。`paper` では `--config-profile conference-preprint` で発表前刷り preset を選べます。
