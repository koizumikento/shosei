...
- `epubcheck` など target/profile 別の実検証強化
- handoff manifest の target/profile 別 metadata 拡張
- target/profile ごとの command-level smoke と public docs の同期の残り
...
GitHub Actions では CLI smoke を `ubuntu-latest`, `macos-latest`, `windows-latest` の 3 OS matrix で回し、VS Code adapter の syntax / test も別 job で継続確認する。
