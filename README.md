...
- `validate --json` による stdout 向け machine-readable surface
- `epubcheck` など target/profile 別の実検証強化
- handoff manifest の target/profile 別 metadata 拡張
- 3 OS smoke の保証内容を README / usage と CI 表示で揃える残り
...
GitHub Actions ではこの smoke を `ubuntu-latest`, `macos-latest`, `windows-latest` の 3 OS matrix で回す。
