...
v0.2 では `book.yml` は見出し文字列の primary source にはしない。
...
- v0.2 では `sections` に `title` や `short_title` の override field を持たせない
...
- `project.type: manga` のとき、`kindle-ja` は将来互換扱いで、v0.2 では非推奨
...
- `weasyprint` は現行 v0.2 経路では `vertical-rl` prose print を表現できないため、`book.writing_mode: vertical-rl` と組み合わせる場合は error とする
- `typst`, `lualatex` は値として受け付ける。v0.2 では default 経路より検証が薄いため、doctor では optional tool として表示し、選択中 engine の場合だけ focused required tools に含める
...
- `cover_pdf: true` は v0.2 では metadata だけ先に定義し、実装は将来に回してもよい
...
- v0.2 では sidecar 自体の merge は行わない
...
- v0.2 では chapter title や section title の metadata は必須ではない
