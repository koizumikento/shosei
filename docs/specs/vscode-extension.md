...
設定編集 UI、Pandoc や PDF engine の独自設定画面は v0.2 の対象外とする。
...
VS Code 側で template / `paper` の場合は profile / repo mode / `series` の場合は初期 book id / title / author / language / output preset を Quick Pick / Input Box で集め、prose では前付き / 後付き scaffold 有無も確認して、`shosei init <path> --non-interactive ...` に変換して実行してよい。
...
- prose で前付き / 後付き scaffold を選んだ場合は `--include-introduction`, `--include-afterword` に変換してよい
...
- `Structure`: config file、prose の chapter list、初期化済み reference workspace file、初期化済み story workspace file、book-scoped な story scene note / structure template file、editorial sidecar file
- `Actions`: project / chapters / reference / story / series の操作を grouped section として整理してよい
...
- 初期表示では `Structure` と `Actions` を優先し、`Context` / `Resolved Config` / `Toolchain` は collapsed で始めてよい
- prose では chapter move / remove を chapter item context menu から起動してよく、add / renumber は command palette と action から起動してよい
- reference surface は command palette と sidebar action から起動してよい
- story surface も command palette と sidebar action から起動してよい
- `Actions` 内の `reference` / `story` / `series` section は collapsed で始めてよい
- `story seed` は book-scoped structure template を選び、`scenes.yml` と scene note 下書きを起こす command として扱ってよい
- sidebar の scene note item から、対応する `scenes.yml` entry を開いて reveal してよい
- `reference map` / `reference check` の対象 workspace が未初期化なら、拡張は `reference scaffold` 実行を提案してよい
- `story map` / `story check` の対象 workspace が未初期化なら、拡張は `story scaffold` 実行を提案してよい

chapter、reference file、story file、story structure template file、sidecar file はクリックで open してよい。
...
### 6.5 `story check` / `story drift`

- `shosei story check` の `dist/reports/*-story-check.json` を読み、issue を Problems に反映してよい
- `shosei story drift` の `dist/reports/*-story-drift.json` も同様に issue を Problems に反映してよい
- `story map` は text/report 確認用とし、Problems には流さない
- `story sync` は同期 command として扱い、Problems 連携は必須にしない
- `story map` / `story check` 実行前に対象 book scope の `story/` または `scenes.yml` がなければ、Problems 連携より先に scaffold 導線を出してよい
...
- v0.2 では sidebar の専用 Tree View を持ってよい

## 7.1 ローカル配布

v0.2 の拡張は Marketplace 公開を前提にしなくてよい。開発中または個人利用では、`editors/vscode/` を VSIX に package して手元の VS Code または Cursor に手動 install してよい。
...
v0.2 では Marketplace 公開の代わりに、GitHub Release に VSIX asset を添付する配布を許容してよい。
...
Open VSX に publish される VSIX には `editors/vscode/README.md` が拡張ページ本文として含まれる。
...
- public README では `shosei` CLI への委譲、拡張が CLI binary / publishing engine を同梱しないこと、必要な CLI install / update、CLI verification command、主要 commands、settings、manual VSIX install 導線を説明してよい
...
- prose / manga の独自 editor を v0.2 で提供すること
- CLI と別系統の状態管理を持つこと
