...
| Kindle Previewer 以外の store / device 固有 validator | advisory future work。`unsupported_checks[]` に記録するが、それだけでは handoff blocker にしない |
...
`delivery_evidence` は提出前に読む evidence summary。`schema_version: 2`, `host_os`, `summary`, `submission_readiness`, `required_ci_checks`, `release_checks`, `manual_checks`, `unsupported_checks` を持つ。required CI で継続確認する local structural / target-profile checks、release host で読む opt-in validator evidence、handoff 前に人間や release operator が埋める manual evidence を分けている。`summary.status` は `passed`, `failed`, `incomplete` のいずれかで、blocking な missing optional tool、skipped validator、manual evidence が残る場合は `incomplete` になる。`summary.ready_for_handoff` と `submission_readiness[]` は enabled target ごとの go/no-go を示す。profile recommendation と advisory な unsupported validator は warning / not-implemented に数えるが、それだけでは handoff blocker にはしない。
...
| Manual / operator evidence | `delivery_evidence.manual_checks[]` と `submission_readiness[]` の missing evidence | handoff 前の残タスクとして解消する |
| Additional store/device validators | advisory な `unsupported_checks[]` | future work として扱うが、それだけでは handoff blocker にしない |
...
`manifest.json` には `build_summary`, `build_stages`, `build_inputs`, `selected_artifacts`, `selected_artifact_details`, `validation_report`, `delivery_evidence`, `git_commit`, `git_dirty`, `dirty_worktree_warning` を含める。`delivery_evidence.summary.ready_for_handoff` が false の場合は package recipient が missing evidence を読める。`proof` では加えて `review_notes`, `review_packet`, `editorial_summary`, `editorial_files` も入り、review packet には delivery readiness と missing delivery evidence も含める。
