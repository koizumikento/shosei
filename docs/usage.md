...
現在の `validate` は local lint と tool availability check が中心で、`epubcheck` や Kindle/print 向け validator の実行結果を深く取り込む段階にはまだ達していない。target/profile ごとの実検証と validator ログの `dist/logs/` 集約は次の remediation で強化する。
