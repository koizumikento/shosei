...
Use the shosei CLI to edit and produce this publishing project.
...
- Add project-specific printer, distribution, naming, or handoff notes here.
...
5. Build or handoff only when the task calls for it.
   ...
   - Use `{{HANDOFF_COMMAND}}` for packaging tasks; choose `kindle`, `print`, or `proof` from the requested destination.
...
- Draft or extend manuscripts and scenes when requested, respecting established canon. Keep proposed canon changes distinct from accepted project facts.
- Do not fabricate factual claims or sources.
